...
import { GIRIS_HEADER_NAV_ITEMS } from './girisNativeData';

function FreshaWordmark() {
  return (
    <svg
      className="gra-hdr-el-4 h-auto fill-current text-foreground w-[75px]"
      data-el-id="hdr-el-4"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 81.8 24"
      pointerEvents="none"
    >
      <title data-el-id="hdr-el-5">Fresha</title>
      <path
        data-el-id="hdr-el-6"
...
function MarketplaceIcon() {
  return (
    <svg className="gra-hdr-el-21" data-el-id="hdr-el-21" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        className="gra-hdr-el-22"
        data-el-id="hdr-el-22"
        fillRule="evenodd"
        clipRule="evenodd"
...
function MenuIcon() {
  return (
    <svg className="gra-hdr-el-28" data-el-id="hdr-el-28" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
      <path
        className="gra-hdr-el-29"
        data-el-id="hdr-el-29"
        fillRule="evenodd"
        clipRule="evenodd"
...
export default function GirisHeader({ menuButtonRef, onMenuToggle }) {
  return (
    <nav
      className="gra-hdr-el-0 mx-auto max-w-screen-desktop px-5 py-3 tablet:h-[72px] laptop:px-8 grid grid-cols-2 items-center gap-3 laptop:grid-cols-[minmax(80px,120px),minmax(380px,auto),minmax(auto,280px)]"
      data-el-id="hdr-el-0"
    >
      <Link className="gra-hdr-el-1" data-el-id="hdr-el-1" to="/giris-native" aria-label="Fresha homepage">
      ...
      <ul className="gra-hdr-el-7 hidden list-none laptop:flex" data-el-id="hdr-el-7">
        {GIRIS_HEADER_NAV_ITEMS.map((item) => (
          <li className={item.type === 'button' ? 'gra-hdr-el-8 relative' : 'gra-hdr-el-16'} data-el-id={item.type === 'button' ? 'hdr-el-8' : 'hdr-el-16'} key={item.key}>
            {item.type === 'button' ? (
              <div className="gra-hdr-el-9 group relative" data-el-id={item.key === 'business-types' ? 'hdr-el-9' : 'hdr-el-13'}>
                <button
                  className={item.key === 'business-types'
                  ...
                  data-el-id={item.key === 'business-types' ? 'hdr-el-10' : 'hdr-el-14'}
                  aria-haspopup="true"
                  aria-expanded="false"
                  type="button"
                >
                ...
                <div
                  className={item.key === 'business-types'
                    ? 'gra-hdr-el-11 absolute left-4 top-full z-50 rounded-[24px] shadow-[0px_8px_32px_0px_rgba(13,22,25,0.16)] bg-white transition-all duration-200 ease-in-out animate-[fade-out_200ms_ease-in-out] invisible opacity-0 max-w-[760px] p-6 px-8 laptop:p-10 desktop:max-w-[900px]'
                    : 'gra-hdr-el-15 absolute left-4 top-full z-50 rounded-[24px] shadow-[0px_8px_32px_0px_rgba(13,22,25,0.16)] bg-white transition-all duration-200 ease-in-out animate-[fade-out_200ms_ease-in-out] invisible opacity-0 max-w-[760px] p-6 px-8 laptop:p-10 desktop:max-w-[900px] min-w-[336px]'}
                  data-el-id={item.key === 'business-types' ? 'hdr-el-11' : 'hdr-el-15'}
                  role="menu"
                  tabIndex="0"
                />
              </div>
      ...
      <div className="gra-hdr-el-18 hidden justify-end items-center gap-4 laptop:flex" data-el-id="hdr-el-18">
        <Link
          ...
          data-el-id="hdr-el-19"
          to="/marketplace-native"
        >
          <div className="gra-hdr-el-20 size-6 transition-colors duration-300 ease-in-out" data-el-id="hdr-el-20">
            <MarketplaceIcon />
          </div>
          <span className="gra-hdr-el-23" data-el-id="hdr-el-23">Marketplace</span>
        </Link>
        ...
        <Link
          ...
          data-el-id="hdr-el-24"
          to="/auth-native"
        >
          Sign up
        </Link>

        <div className="gra-hdr-el-25" data-el-id="hdr-el-25">
          <button
            ref={menuButtonRef}
